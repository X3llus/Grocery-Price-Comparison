<script>
	...
	let state = 0; // 0 = initial, 1 = sign in, 2 = register, 3 = reset password
	...
	<div class="flex flex-col w-min h-min p-32 border-2 rounded-3xl shadow-xl m-auto bg-background">
		{#if state === 0}
			<div class="space-y-4">
				...
				<button
					on:click={() => (state = 1)}
					class="btn btn-primary w-56 bg-primary text-white p-4 text-center rounded-full  flex justify-around"
		...
		{:else if state === 1}
			...
			<div class="space-y-4">
				...
				<button
					...
					class="btn btn-primary w-56 bg-primary text-white p-1 text-center rounded-full"
					>Sign in</button
				>
				<button
					on:click={() => (state = 3)}
