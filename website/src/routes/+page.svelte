<script>
	...
	const images = [
		...
		},
		{
			...
			content:
				"Loblaws Inc. is a Canadian supermarket chain and is a subsidiary of Loblaw Companies Limited. It is Canada's largest food distributor.",
		},
		{
			url: '/metro.jpg',
			description: 'Metro Logo',
			content:
				'Metro Inc. is a Canadian food retailer operating in the provinces of Quebec and Ontario. The company is based in Montreal, Quebec and is the third largest grocer in Canada.',
		},
	];
	...
	const faq = [
		...
		},
		{
			...
			answer:
				'It is only necessary to make an account with us if you want to use our service to save lists or favourite products. Our simple look-up does not require an account.',
		},
		{
			question: 'Will you be adding more stores?',
			answer:
				'We are constantly working on expanding the number of stores available to our customers! Sadly we do not have any estimates due to the complexity of the process we have to undertake for each store.',
		},
		{
			question: 'Do you have a mobile app?',
			answer:
				"Currently we don't have an app, but our site is fully optimized for mobile browsing!",
		},
	];
			...
			<div class="mx-auto justify-center gap-10 flex flex-col xl:flex-row px-5 pb-10 items-center">
				<div class="flex w-3/4 lg:w-1/3 items-center">
					...
					<p class=" text-xl text-white font-sans font-medium">
						Searching our database for generic or specific brands of products and finding the best
						deal that is local to you.
					</p>
				</div>
				...
				<div class="flex w-3/4 xl:w-1/3 items-center">
					...
					<p class=" text-xl text-white font-sans font-medium">
						The creation of user accounts lets you see your search history, enables you to save
		...
		<div data-aos="fade-up" data-aos-once="true">
			...
			<div class="mx-auto flex flex-wrap gap-10 justify-center px-5 py-10">
				{#each images as src}
					...
					>
						<img
							class="h-auto max-w-xs sm:max-w-sm rounded-xl duration-300 group-hover:blur-sm group-hover:opacity-20"
							src={src.url}
							alt={src.description}
						/>
						<p
							class="absolute opacity-0 group-hover:opacity-100 duration-500 text-white max-w-xs sm:max-w-sm text-sm sm:text-base"
						>
							{src.content}
						</p>
					</div>
		...
		<div data-aos="fade-up" data-aos-once="true">
			<div class=" flex justify-center">
				<div class="w-7/8 md:w-3/4 xl:w-1/2">
					<h2 class="text-6xl p-5 font-sans font-medium text-white sm:text-left text-center">
						FAQ
					</h2>
